...
sed -i "/kk_KZ.UTF-8/s/^#//" /etc/locale.gen
locale-gen
...
echo -e "${user_passphrase}\n${user_passphrase}" | passwd ${username}
passwd --delete root && passwd --lock root # disable the root user
sed -i "/%wheel ALL=(ALL:ALL) ALL/s/^#//" /etc/sudoers # give the wheel group sudo access
...
sed -i '/^GRUB_CMDLINE_LINUX_DEFAULT=/s/\".*\"/\$boot_options/' /etc/default/grub
