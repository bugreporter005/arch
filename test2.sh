...
sed -i "/GRUB_CMDLINE_LINUX_DEFAULT=/s/\"$/rd.luks.name=${ROOT_UUID}=${luks_label} rd.luks.options=tries=3,discard,no-read-workqueue,no-write-workqueue root=\/dev\/mapper\/${luks_label} rootflags=subvol=\/@ rw cryptkey=rootfs:\/root\/.cryptkey\/keyfile.bin loglevel=3 rd.udev.log_priority=3\"/" /mnt/etc/default/gru
